...
`include "builddate.v"
	...
	wire	[7:0]	wb_skip; // bits 011005f0, sbaw=23
	assign	wb_skip = {
			wb_addr[22],
			wb_addr[18],
			wb_addr[8],
	...
	assign	scope_sdcard_sel = ( wb_skip[ 7: 0] ==  8'b0010_1000_);
	assign	       flctl_sel = ( wb_skip[ 7: 0] ==  8'b0010_1001_);
	assign	        uart_sel = ( wb_skip[ 7: 0] ==  8'b0010_1010_);
	assign	         cfg_sel = ( wb_skip[ 7: 3] ==  5'b0011_0);
	assign	       bkram_sel = ( wb_skip[ 7: 6] ==  2'b01);
	assign	       flash_sel = ( wb_skip[ 7: 7] ==  1'b1);
	assign	         dio_sel = ( wb_skip[ 7: 2] ==  6'b0010_01);
	assign	         sio_sel = ( wb_skip[ 7: 1] ==  7'b0010_000);
	assign	none_sel = (wb_stb)&&({ sio_sel, dio_sel, flash_sel, bkram_sel, cfg_sel, uart_sel, flctl_sel, scope_sdcard_sel} == 0);
	//
	// many_sel
	//
...
`ifdef	VERILATOR
	...
	always @(*)
		case({sio_sel, dio_sel, flash_sel, bkram_sel, cfg_sel, uart_sel, flctl_sel, scope_sdcard_sel})
			8'h0: many_sel = 1'b0;
...
`endif	// VERILATOR
	...
	always @(posedge i_clk)
		case({sio_ack, dio_ack, flash_ack, bkram_ack, cfg_ack, uart_ack, flctl_ack, scope_sdcard_ack})
			8'h0: many_ack <= 1'b0;
	...
	always @(posedge i_clk)
		dio_ack <= pre_dio_ack;
	always @(posedge i_clk)
		wb_ack <= (wb_cyc)&&(|{sio_ack, dio_ack, flash_ack, bkram_ack, cfg_ack, uart_ack, flctl_ack, scope_sdcard_ack});
...
`ifdef	RTCDATE_ACCESS
	...
	// The Calendar DATE
	//
	rtcdate	thedate(i_clk, rtc_ppd, wb_cyc,
...
`endif	// SDSPI_ACCESS
	...
	always @(posedge i_clk)
	begin
		casez({ sio_ack, dio_ack, flash_ack, bkram_ack, cfg_ack, uart_ack, flctl_ack, scope_sdcard_ack })
			8'b1???????: wb_idata <= sio_data;
			8'b01??????: wb_idata <= dio_data;
			8'b001?????: wb_idata <= flash_data;
			8'b0001????: wb_idata <= bkram_data;
			8'b00001???: wb_idata <= cfg_data;
			8'b000001??: wb_idata <= uart_data;
			8'b0000001?: wb_idata <= flctl_data;
			8'b00000001: wb_idata <= scope_sdcard_data;
